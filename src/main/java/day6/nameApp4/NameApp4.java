...
//        p1.age = 0;
        ...
        while (true) {
            ...
            if (commend.equals("help")) {
                ...
                // 입력값 저장 ** 중요
                // 지역 변수 = > 특정 지역에서 만들어져서 지역이 끝나면 사라진다.
                //new Person(); > 객체가 만들어짐 하지만 리모컨이 저장되지 않는 상태임
                Person p1 = new Person();
                peopleList.add(p1);
                p1.name = name ;
                p1.age = age;
                // p1변수가 사라지기 전에 p1에 저장된 person 리모콘을 배열에 저장
                // 한번 만들어진 객체는 덮어써지는 것이 아니라 객체로 따로 만들어져있고
                // 담아두는 배열이 없으면 리모컨을 없애는 것이다.
                // 그래서 배열안에 담아두고 인덱스로 부르면 된다.

                System.out.println(peopleList.get(count).name + "이름을 저장했습니다.");
                count++;
                ...
                while (output <= count - 1) {
                    System.out.println((output + 1) + ". " + peopleList.get(output).name+", "+peopleList.get(output).age);
                    Person boul = peopleList.get(output);
                    System.out.println((output + 1) + ". " + boul.name+", "+boul.age);
                    output++;
                }
                System.out.println("=====================");
